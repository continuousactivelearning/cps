...
def generate_response(prompt: str, system_prompt: str = None) -> str:
    """Generate a response using Groq Cloud's Mistral-7B-Instruct model"""
    ...
    try:
        ...
        data = {
            "model": "mistral-saba-24b",  # Groq's Mistral-7B-Instruct model
            "messages": [
...
def get_dsa_recommendations(topic: str) -> Dict:
    """Get DSA topic explanation and video recommendations"""
    try:
        ...
        print(f"🔍 Finding video resources for: {topic}")
        ...
        print(f"❌ Error in get_dsa_recommendations: {str(e)}")
        return {
            "topic": topic,
...
def main():
    ...
    print("🚀 Starting DSA Learning Assistant with Groq Cloud...")
